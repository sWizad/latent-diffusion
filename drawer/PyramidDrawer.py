...
class PyramidDrawer(nn.Module):
    def __init__(self,stages,initial_image=None, color_space =  "YCoCg"):
        ...
        if initial_image is not None:
            ...
            for in_level  in [-2, -5, -8]:
                image = torch.nn.functional.interpolate(image, size=self.dims[in_level], mode='bilinear', align_corners=False)

                img_channels = self.imageToParams(image)
                pix = []
                for channel in range(3):
                    c = img_channels[channel] * 0.25
                    ...
                    pix.append(param_pix)
                params_pyramid[in_level] = pix
        else:
            dim_len = len(self.dims)
            for i, dim in enumerate(self.dims):
                if color_space == "YCoCg":
                    luma = (torch.randn(size = (1,1,dim[0], dim[1])) * luma_noise_scale * luma_noise_persistence**i / dim_len).clamp(-init_noise_clamp / dim_len, init_noise_clamp / dim_len) 
                    Co = (torch.randn(size = (1,1,dim[0], dim[1])) * chroma_noise_scale * chroma_noise_persistence**i / dim_len).clamp(-init_noise_clamp / dim_len, init_noise_clamp / dim_len) 
                    Cg = (torch.randn(size = (1,1,dim[0], dim[1])) * chroma_noise_scale * chroma_noise_persistence**i / dim_len).clamp(-init_noise_clamp / dim_len, init_noise_clamp / dim_len) 
                    ...
                    pix = []
                    for channel in range(3):
                        pix_c = (torch.randn(size = (1,1,dim[0], dim[1])) * chroma_noise_scale * chroma_noise_persistence**i / dim_len).clamp(-init_noise_clamp / dim_len, init_noise_clamp / dim_len) 
                        param_pix = torch.nn.parameter.Parameter( pix_c.cuda(), requires_grad=True)
                        pix.append(param_pix)
                params_pyramid.append(pix)
    ...
    def update_optim(self,stage):
        for i in range(len(self.dims)):
            if self.color_space == "YCoCg":
                ...
                self.optimizer.param_groups[i*3+1]["lr"] = stage["lr_chroma"] * stage["lr_scales"][i]
                self.optimizer.param_groups[i*3+2]["lr"] = stage["lr_chroma"] * stage["lr_scales"][i]
            elif self.color_space == "RGB":
                self.optimizer.param_groups[i*3]["lr"] = stage["lr_luma"] * stage["lr_scales"][i]
